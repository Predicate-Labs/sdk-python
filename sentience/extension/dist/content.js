...
(function () {
  ...
  window.addEventListener('message', (event) => {
    ...
    switch (event.data.type) {
      ...
      case 'SENTIENCE_SHOW_GRID_OVERLAY':
        handleShowGridOverlay(event.data);
        break;
    }
  });
   ...
   * @param {Object} data - Message data with grids and targetGridId
   */
  function handleShowGridOverlay(data) {
    const { grids, targetGridId } = data;

    if (!grids || !Array.isArray(grids)) {
      console.warn('[Sentience Bridge] showGridOverlay: grids must be an array');
      return;
    }
    ...
    grids.forEach((grid) => {
      ...
      const isTarget = grid.grid_id === targetGridId;
      const isDominant = grid.is_dominant === true;

      // Grid colors: Red for target, Orange for dominant, Purple for regular
      let color = '#9B59B6'; // Purple (default)
      if (isTarget) {
        color = '#FF0000'; // Red for target
      } else if (isDominant) {
        color = '#FF8C00'; // Orange for dominant group
      }
      const borderStyle = isTarget ? 'solid' : 'dashed'; // Dashed for grids
      const borderWidth = isTarget ? 3 : isDominant ? 2.5 : 2;
      const opacity = isTarget ? 1.0 : isDominant ? 0.9 : 0.8;
      const fillOpacity = opacity * 0.1; // 10% opacity for grids (lighter than elements)
      ...
      // Add badge with grid_id and label
      let labelText = grid.label ? `Grid ${grid.grid_id}: ${grid.label}` : `Grid ${grid.grid_id}`;
      // Add dominant indicator if this is the dominant group
      if (grid.is_dominant) {
        labelText = `⭐ ${labelText} (dominant)`;
      }
      const badge = document.createElement('span');
      ...
      // Add target indicator if target
      if (isTarget) {
        const targetIndicator = document.createElement('span');
        targetIndicator.textContent = '🎯';
        targetIndicator.style.cssText = `
    ...
    console.log(`[Sentience Bridge] Grid overlay shown for ${grids.length} grids`);

    // Auto-remove after 5 seconds
    overlayTimeout = setTimeout(() => {
      removeOverlay();
      console.log('[Sentience Bridge] Grid overlay auto-cleared after 5 seconds');
    }, 5000);
  }
